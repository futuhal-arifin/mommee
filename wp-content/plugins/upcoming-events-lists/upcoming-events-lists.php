...
class Upcoming_Events_Lists {
	...
	public function render_event_info_metabox( $post ) {
		...
		<p>
			<label for="sis-event-address"><?php _e( 'Event Address:', 'upcoming-events' ); ?></label>
			<input type="text" id="sis-event-address" name="sis-event-address" class="widefat" value="<?php echo $event_address; ?>" placeholder="eg. Jl. TB. Simatupang No. 8, Cilandak Barat, Jakarta 12430">
		</p>
		<?php
	}
	...
	function save_event_info( $post_id ) {
		...
		if ( isset( $_POST['sis-event-address'] ) ) {
			update_post_meta( $post_id, 'event-address', sanitize_text_field( $_POST['sis-event-address'] ) );
		}
	}
	...
	function custom_columns_head( $defaults ) {
		...
		$defaults['event_address'] = __( 'Address', 'upcoming-events' );
	...
	function custom_columns_content( $column_name, $post_id ) {
		...
		if ( 'event_address' == $column_name ) {
			$venue = get_post_meta( $post_id, 'event-address', true );
			echo $venue;
		}
	}
	...
	function upcoming_events_single_content( $content ){
		if ( is_singular('event') || is_post_type_archive('event') ) {
			...
			$event_address = get_post_meta( get_the_ID(), 'event-address', true );
			...
			$event .= '<td><strong>'.__('Event Address:', 'upcoming-events').'</strong><br>'.$event_address.'</td>';
			$event .= '</tr>';
			$event .= '</table>';
